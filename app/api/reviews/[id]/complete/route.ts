...
import { validateRequest, completeJobSchema } from '@/lib/validation';
...
async function emit(event: string, payload: any) {
  try {
    ...
    if (axiom) {
      await fetch('https://api.axiom.co/v1/datasets/cloudcurio/ingest', { 
        method: 'POST', 
        headers: { 
          'Authorization': `Bearer ${axiom}`, 
    ...
    if (phKey) {
      await fetch(`${phHost}/capture/`, { 
        ...
        body: JSON.stringify({ api_key: phKey, event, properties: payload }) 
      });
    }
  } catch {}
}
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const validation = validateRequest(completeJobSchema, body);
  
  if (!validation.success) {
    return validation.error;
  }
  
  const { status, content, error, gpu } = validation.data;

  if (content) {
    await prisma.reviewArtifact.upsert({ 
      where: { jobId: params.id }, 
      create: { jobId: params.id, content }, 
      update: { content } 
    });
  }
